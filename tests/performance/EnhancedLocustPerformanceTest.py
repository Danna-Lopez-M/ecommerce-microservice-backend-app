...
class EcommerceUserBehavior(SequentialTaskSet):
    ...
    def register_user(self):
        ...
        with self.client.post("/user-service/api/users/register", 
                             json=payload,
    ...
    def login_user(self):
        ...
        with self.client.post("/user-service/api/auth/login",
                             json=payload,
                             catch_response=True) as response:
    ...
    def browse_products(self):
        ...
        url = f"/product-service/api/products?{filter_str}" if filter_str else "/product-service/api/products"
    ...
    def search_products(self):
        ...
        with self.client.get(f"/product-service/api/products/search?query={query}",
                            headers=headers,
                            catch_response=True) as response:
            if response.status_code == 200:
                response.success()
    ...
    def view_product_details(self):
        ...
        with self.client.get(f"/product-service/api/products/{self.product_id}",
                            headers=headers,
                            catch_response=True) as response:
            if response.status_code == 200:
                response.success()
