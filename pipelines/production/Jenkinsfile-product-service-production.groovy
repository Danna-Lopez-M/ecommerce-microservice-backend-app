            ...
            steps {
                script {
                    sh '''
                        ...
                        curl -f ${SERVICE_URL}/product-service/actuator/info || echo "Info endpoint not available"
                        
                        # Metrics endpoint
                        echo "3. Metrics Endpoint..."
                        curl -f ${SERVICE_URL}/product-service/actuator/metrics || echo "Metrics endpoint not available"
                        
                        # Prometheus endpoint
                        echo "4. Prometheus Endpoint..."
                        curl -f ${SERVICE_URL}/product-service/actuator/prometheus || echo "Prometheus endpoint not available"
                    ...
                    script {
                        sh '''
                            ...
                            mvn test -Dtest=**/*E2ETest \
                                ...
                                -Dservice.context.path=/product-service || true
                        '''
                    }
                }
            }
...
stats_file = os.environ.get('PERFORMANCE_RESULTS_DIR', 'performance-results') + '/' + \
             os.environ.get('SERVICE_NAME', 'product-service') + '-production-performance_stats_stats.csv'
                ...
                script {
                    sh '''
                        ...
                        for i in {1..5}; do
                            ...
                            if curl -f ${SERVICE_URL}/product-service/actuator/health; then
